...
function AddPagination(count) {
  ...
  for (let i = 0; i <= cunt; i++) {
    slider_pagination.insertAdjacentHTML(
      "afterbegin",
      `<span data-id='${i}' class="slider-img-r-pagination-span"></span>`
    );
  }
}
...
pagination.forEach((i) =>
  i.addEventListener("click", (event) => MovePagination(event))
);
...
const product_width =
  Number(product_box.offsetWidth) - Number(product_part.offsetWidth);
